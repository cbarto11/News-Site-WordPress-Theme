 ...
 *
 */
class Exchange_FeaturedAdminPage
{
	...
	public function show_page()
	{
		...
		$featured_stories_options = new Exchange_FeaturedStoriesOptions;
		$options = $featured_stories_options->get_options();
		...
		foreach( $this->COLUMNS as $column_id ):
			...
			foreach( $column as $section_id ): 
				...
				<?php for($i = 0; $i < $section->featured_num_stories; $i++): ?>
		...
		<input type="submit" name="set_featured_stories" value="Update Stories" />
	...
	private function process_post()
	{
		...
		if( !isset($_POST['set_featured_stories']) ) { return; }
		...
		foreach( $this->COLUMNS as $column_id )
		{
			...
			foreach( $column as $section_id )
			{
				...
				while( count($options[$section->key]) < $section->featured_num_stories )
				{
		...
		$featured_stories_options = new Exchange_FeaturedStoriesOptions;
		$featured_stories_options->save_options( $options );
	}
}
